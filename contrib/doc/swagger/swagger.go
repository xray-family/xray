...
func CustomWrapHandler(config *Config, handler *webdav.Handler) xray.HandlerFunc {
	...
	return func(ctx *xray.Context) {
		...
		var req = ctx.Request.Raw.(*http.Request)
		matches := matcher.FindStringSubmatch(req.RequestURI)

		if len(matches) != 3 {
			_ = ctx.WriteString(http.StatusNotFound, http.StatusText(http.StatusNotFound))
		...
		case "doc.json":
			...
			if err != nil {
				_ = ctx.WriteBytes(http.StatusInternalServerError, nil)
				return
			}

			_ = ctx.WriteString(http.StatusOK, doc)
		default:
			handler.ServeHTTP(ctx.Writer.Raw().(http.ResponseWriter), ctx.Request.Raw.(*http.Request))
		}
	}
}
